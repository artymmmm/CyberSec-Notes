CSRF (Cross-Site Request Forgery, межсайтовая подделка запроса) — тип атаки, заставляющий авторизованного пользователя выполнить нежелательные действия (например, смена пароля, перевод средств) на доверенном сайте без его ведома. Злоумышленник использует автоматическую отправку cookies браузером, используя поддельные ссылки или скрипты.
...
- SameSite: механизм браузера, определяющий, будет ли браузеры отправлять cookie при кросс-доменных (сторонних) запросах. Значения атрибута SameSite:
	- Strict: браузер не отправляет куки на сайт, который их установил, при кросс-доменных запросах
	- Lax (выставляется по умолчанию, если не указан атрибут SameSite): браузер отправляет куки при кросс-доменных запросах при выполнении условий:
		- Запрос использует метод GET
		- Запрос - навигация верхнего уровня (овый сайт полностью заменяет текущий в окне или вкладке браузера), выполненная пользователем (клик по ссылке, ввод ссылки в браузерную строку, использование закладки, перенаправление (HTTP Redirect), инициированное сервером)
	- None: браузер отправляет куки на сайт, который их установил, при любых запросах
![[SameSite.jpg]]
- Referer-based validation: проверка того, что запрос отправлен с сайта того же домена
## Способы обхода CSRF токена
...
- Выпуск новых куки без указания SameSite (по умолчанию применяется SameSite=Lax) имеет особенность: в течение 120 секунд после выпуска куки верхнеуровневые POST-запросы отправляют куки вместе с запросом с любого сайта. Атакующий может использовать функциональность сайта (например, OAuth-авторизация), позволяющую выпустить пользователю новые куки, после чего использовать их в течение 120 секунд с помощью POST-запроса