...
SQL-инъекция первого порядка происходит, когда приложение обрабатывает ввод пользователя из HTTP-запроса и небезопасным образом включает этот ввод в SQL-запрос.
SQL-инъекция второго порядка происходит, когда приложение получает ввод пользователя из HTTP-запроса и сохраняет его для дальнейшего использования. Обычно это делается путем размещения ввода в базе данных, но уязвимость не возникает в БД. Позже, при обработке другого HTTP-запроса, приложение извлекает сохраненные данные и небезопасным образом включает их в SQL-запрос. По этой причине SQL-инъекция второго порядка также известна как хранимая SQL-инъекция.