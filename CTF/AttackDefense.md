...
- Веб-сервры: `/var/log/apache2/`, `/var/log/nginx/`
...
Слушает `16667` без шифрования, конектится на `127.0.0.1:6667`. Меняет от клиента к серверу `hello`→`preved`, а от сервера к клиенту - любые флаги по регулярке на `CENSORED`.
